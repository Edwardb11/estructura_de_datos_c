...
#define t2 "UCATECI"
#define t3 "Proyecto Final de Estructura de Datos"
#define t4 "Facilitador de teoria: Diego Orlando Matias"
#define t5 "Facilitador de laboratorio: Jose Ramon Capellan"
#define t6 "M e n u   P r i n c i p a l"
...
int login (){
	...
	if (acceder == false){
        ...
        cuadro(18,102,1,28);
        setcolor(WHITE);
        iraxy(25,15);printf("Has superado el limite de tres intentos, intente m%cs tarde!",160);
		Sleep(2000);
		return 6;
	}else{
        system("CLS");
        ...
        setcolor(WHITE);
        setcolor(WHITE);
        iraxy( (130-strlen(t5))/2, 10); printf("2019-0890   \t\tEnmanuel Ruiz\n");
        iraxy( (130-strlen(t5))/2, 13); printf("2019-0400   \t\tYudermi Hern%cndez\n\n",160);
        for(int i=25; i<95; i++){
            iraxy(((102-strlen(t5))/2)+5, 19); printf("CARGANDO...\n\n");
            iraxy(i,21); setcolor(CYAN); printf ("%c",219);
            Sleep(70);
	    }
        Sleep(200);
        // for(int i=25; i<95; i++){
        //     iraxy( (130-strlen(t5))/2, 13); printf("CARGANDO...\n\n");
        //     iraxy(i,16); setcolor(CYAN); printf ("%c",219);
        //     Sleep(100);
	    // }
        return 1; 
	}	
...
int main(){
    ...
    if(Opcion != 6){
        ...
        setcolor(YELLOW);
        iraxy( (120-strlen(t1))/2, 14);printf(t1);
        iraxy( (120-strlen(t2))/2, 15);printf(t2);
        time_t  tiempoahora;
        time(&tiempoahora);
        printf("\n\t\t\t\t\t\t%s", ctime(&tiempoahora));
        struct tm* mitiempo = localtime(&tiempoahora); 
        printf("\t\t\t\t\t\t\t%d/%d/%d", mitiempo->tm_mday,mitiempo->tm_mon+1, mitiempo->tm_year+1900);
        iraxy( (120-strlen(t3))/2, 18);printf(t3);
        iraxy( (120-strlen(t4))/2, 19);printf(t4);
        iraxy( (120-strlen(t5))/2, 20);printf(t5);
        getch();
    }
    while(Opcion != 6){
        system("CLS");
        ...
        {
            setcolor(WHITE);
            iraxy( (120-strlen(t6))/2, 10);printf(t6);
...
void InsertarCuenta(void){
    int nRegistros=0,Opcion=1,Tecla,comp,Balance;
    ...
    {    
        ...
        time(&tiempoahora);
        struct tm* mitiempo = localtime(&tiempoahora); 
        sprintf(Fecha, "%d/%d/%d", mitiempo->tm_mday,mitiempo->tm_mon+1, mitiempo->tm_year+1900);
        iraxy(30,12);printf("Fecha: %s",Fecha);
        // gets(Fecha);
        fflush(stdin);
        ...
        {
            case 1:
                strcpy (AUX_C->TipoMovimiento,"Deposito"); 
                break;
            case 2: 
                strcpy (AUX_C->TipoMovimiento,"Retiro"); 
                break;
        }
        fflush(stdin);
        ...
        if (nRegistros==1)
        {
            ...
            if (comp==0){
                Balance = AUX_E->Balance + Monto;
                AUX_E->Balance = Balance;
            }
            ...
            cuadro(18,102,1,28);
            setcolor(WHITE);
            iraxy(22,13);printf("Transaccion exitosa %s",AUX_E->Nombre);
            iraxy(22,15);printf("...");
            getch();
            break;
        }
        else
        {
            ...
            cuadro(18,102,1,28);
            setcolor(WHITE);
            iraxy(22,13);printf("No existe ese Empleado");
            iraxy(22,14);printf("Pulse cualquier tecla para volver al insertar los datos");
            iraxy(22,15);printf("...");
            getch();
        }
    }
...
void consultarEmpleados(void){
    ...
    if (CAB_E==NULL){
        ...
        iraxy(20,7);printf("---------------------------------------------------------------------------------");
        setcolor(BLUE);
        iraxy(20,8);printf("| ID | Nombre                   | Tel%cfono     | Ciudad               | Balance |",130);
        setcolor(WHITE);
        iraxy(20,8);printf("|");
        iraxy(25,8);printf("|");
        iraxy(52,8);printf("|");
        ...
        while (AUX_E != NULL)
        {
            ...
            iraxy(90,y);printf("| %.2f",AUX_E->Balance);
            iraxy(100,y);printf("|");
            y++;
            AUX_E=AUX_E->ant;
        }
...
void consultarCuenta(void){
    ...
    if (CAB_C==NULL){
        ...
        iraxy(20,7);printf("---------------------------------------------------------------------------------");
        setcolor(BLUE);
        iraxy(20,8);printf("| No.Cuenta | Empleado | Fecha    | Tipo Movimiento | Monto          | Concepto |");
        setcolor(WHITE);
        iraxy(20,8);printf("|");
        iraxy(32,8);printf("|");
        iraxy(43,8);printf("|");
        ...
        while (AUX_C != NULL)
        {
            ...
            iraxy(43,y);printf("| %s",AUX_C->Fecha);
...
void eliminarEmpleado(int IDEmpleadoBusqueda){
    int y=10,nRegistros=0,comp,Opcion = 1, Tecla,mensaje;
    char nombreBusqueda[25];
    ...
    if (nRegistros==1)
    {
        ...
        if(mensaje==0){
            ...
            if(mensaje==1){
                system("CLS");
                cuadro(18,102,1,28);
                iraxy(30,8);printf("El empleado ID%d aun debe pagar, no puede ser borrado",AUX_E->IDEmpleado);
                getch();
            }
        }
    }
